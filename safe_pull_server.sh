...
if [ -n "$MODIFIED_FILES" ]; then
    ...
    if echo "$MODIFIED_FILES" | grep -q "safe_pull_server.sh"; then
        ...
        git checkout -- safe_pull_server.sh 2>/dev/null || true
        echo "   ✅ Локальные изменения отменены"
    fi
    
    # Для других критических файлов тоже можно отменить, если нужно
    # Но пока оставляем только safe_pull_server.sh
fi
...
if ! git pull origin main 2>&1; then
    ...
    # Если ошибка из-за локальных изменений, отменить их
    if git status 2>/dev/null | grep -q "modified:"; then
        echo "   Отменяю все локальные изменения в отслеживаемых файлах..."
        git checkout -- . 2>/dev/null || true
        echo "   Повторная попытка pull..."
        git pull origin main 2>&1 || {
            echo "   ❌ Не удалось выполнить git pull после отмены изменений"
            echo "   Пожалуйста, разрешите конфликты вручную:"
            echo "     1. git status"
            echo "     2. git checkout -- <файл>  (для отмены изменений)"
            echo "     3. git pull origin main"
            exit 1
        }
    else
        echo "   ❌ Не удалось автоматически разрешить проблему"
        echo "   Пожалуйста, разрешите конфликты вручную"
        exit 1
    fi
fi
